  ...
  template: `
    <form class="auth-form" [formGroup]="form" (ngSubmit)="submit()" novalidate>
      <div class="auth-copy">
        ...
        <p>Use at least 10 characters with one number.</p>
      </div>
...
export class ResetPasswordPageComponent {
  ...
  protected submit(): void {
    ...
    if (this.form.invalid || this.loading || !this.token) {
      this.submitError = !this.token ? 'The reset link is missing its token.' : '';
      return;
    }
    ...
    this.auth
      ...
      .subscribe({
        ...
        error: () => {
          this.submitError = 'This reset link is invalid or expired. Request a new one.';
        },
      });
  }
}
